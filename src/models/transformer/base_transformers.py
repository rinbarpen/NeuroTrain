...
class _MLP(nn.Module):
    def __init__(self, embed_dim: int, r: int, mlp_dropout: float=0.2, mlp_act=nn.LeakyReLU, mlp_bias=False):
        super(_MLP, self).__init__()

        self.embed_dim = embed_dim
        self.mlp = nn.Sequential(
            nn.Linear(embed_dim, embed_dim * r, bias=mlp_bias),
            nn.Dropout(mlp_dropout, inplace=True),
            mlp_act(inplace=True),
            nn.Linear(embed_dim * r, embed_dim, bias=mlp_bias),
        )
...
# if pre_norm is True(Pre-LN Arch), recommend qkv_bias=False
# if pre_norm is False(Post-LN Arch), recommend qkv_bias=True
class TransformerEncoder(nn.Module):
    def __init__(self, n_layers: int, embed_dim: int, num_heads: int, r: int=4, attn_dropout: float=0.2, attn_out_dropout: float=0.0, mlp_dropout: float=0.3, mlp_out_dropout: float=0.3, mlp_act=nn.LeakyReLU, num_groups: int|None=None, attn_type: AttentionType='mha', qkv_bias=False, mlp_bias=False, rms_norm=False, *, share_kv=False, rope=False, pre_norm=True):
        ...
        self.mlp = _MLP(embed_dim=embed_dim, r=r, mlp_dropout=mlp_dropout, mlp_act=mlp_act, mlp_bias=mlp_bias)
...
# if pre_norm is True(Pre-LN Arch), recommend qkv_bias=False, mlp_bias=False
# if pre_norm is False(Post-LN Arch), recommend qkv_bias=True, mlp_bias=True
...
def get_cfg(mode: str, dropout: tuple[float, float, float, float]=(0.2, 0.2, 0.3, 0.3), num_groups: int|None=None, mlp_act: type[nn.Module]=nn.SiLU, rms_norm=True, pre_norm=True, rope=False, share_kv=False) -> dict:
    if num_groups:
        attn_type = 'mha'
    else:
        attn_type = 'gqa'
    ...
    c = dict(
        ...
        attn_type=attn_type,
        qkv_bias=not pre_norm,
        mlp_bias=not pre_norm,
        rms_norm=rms_norm,
        share_kv=share_kv,
        rope=rope,
        pre_norm=pre_norm)
