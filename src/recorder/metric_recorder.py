...
class MetricRecorder:
    ...
    def finish_one_batch(self, targets: np.ndarray, outputs: np.ndarray):
        ...
        # 使用新的 many_metrics 函数计算指标
        metrics = many_metrics(
            metric_fns, 
            targets, 
            outputs, 
            class_split=True,  # 按类别分别计算
            class_axis=1
        )
        ...
        # 重新组织结果格式以匹配原有的数据结构
        result = {}
        for i, metric_label in enumerate(self.metric_labels):
            result[metric_label] = {}
            actual_name = name_mapping[metric_label]
            metric_scores = metrics[actual_name]
            for j, class_label in enumerate(self.class_labels):
                result[metric_label][class_label] = metric_scores[j]
                self.all_metric_label_scores[metric_label][class_label].append(metric_scores[j])
