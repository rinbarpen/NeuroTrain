...
class StareDataset(CustomDataset):
    ...
    def __init__(self, base_dir: Path, dataset_type: Literal['train', 'test'], is_rgb: bool = False, **kwargs):
        ...
        Args:
            ...
            **kwargs: 预留扩展参数
        """
        super(StareDataset, self).__init__(base_dir, dataset_type, **kwargs)
    ...
    def to_numpy(save_dir: Path, base_dir: Path, betweens: Betweens, **kwargs):
        ...
        train_dataset = StareDataset.get_train_dataset(base_dir, **kwargs)
        test_dataset = StareDataset.get_test_dataset(base_dir, **kwargs)
    ...
    def get_train_dataset(base_dir: Path, **kwargs):
        """获取训练集实例"""
        return StareDataset(base_dir, 'train', **kwargs)
    ...
    def get_test_dataset(base_dir: Path, **kwargs):
        """获取测试集实例"""
        return StareDataset(base_dir, 'test', **kwargs)
