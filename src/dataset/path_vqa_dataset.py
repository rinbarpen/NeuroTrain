...
class PathVQADataset(CustomDataset):
    ...
    def __init__(self, base_dir: Path, split: Literal['train', 'test', 'valid']):
        """
        初始化 PathVQADataset。

        Args:
            base_dir (Path): 数据集所在的根目录。
            split (Literal['train', 'test', 'valid']): 数据集划分（训练、测试或验证）。
        """
        super(PathVQADataset, self).__init__(base_dir, split)
        data_path_pattern = base_dir / self.mapping[split]
...
def get_pathvqa_dataloader(base_dir: Path, processor, batch_size: int, split: Literal['train', 'test', 'valid']='train', num_workers: int = 4):
    """
    ...
    """
    dataset = PathVQADataset(base_dir, split, processor)
