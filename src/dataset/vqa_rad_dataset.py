...
class VQARadDataset(CustomDataset):
    ...
    def __init__(self, base_dir: Path, split: Literal['train', 'test'], processor):
        super(VQARadDataset, self).__init__(base_dir, split)
        data_path = base_dir / self.mapping[split]
...
def get_vqarad_dataloader(base_dir: Path, processor, batch_size: int, split: Literal['train', 'test']='train', num_workers: int = 4):
    dataset = VQARadDataset(base_dir, split, processor)
