...
class ISIC2016ReasoningSegDataset(CustomDataset):
    ...
    def __init__(self, root_dir, split: Split, task_type: str="Task1", processor=None):
        super(ISIC2016ReasoningSegDataset, self).__init__(root_dir, split)
        data_path_pattern = Path(self.root_dir) / self.mapping[self.dataset_type].format(task_type=task_type)
