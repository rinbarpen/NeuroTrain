...
class Net(nn.Module):
    ...
    def __init__(self, n_classes: int, model_name: str='sam_vit_b', image_size: int|tuple[int, int]=224, patch_size: int|tuple[int, int]=14, window_size: int=1, num_mask_tokens: int=1):
        ...
        self.prompt_encoder = PromptEncoder(embed_dim=embed_dim, patched_image_size=patched_image_size, mask_channels=embed_dim, n_classes=n_classes, n_tokens_per_class=num_mask_tokens)

        self.mask_decoder = MaskDecoder(n_classes=n_classes, image_size=self.image_size, embed_dim=embed_dim, patch_size=self.patch_size)

    def forward(self, depth_x: torch.Tensor):
        # (B, C, H, W, D)
        if depth_x.dim() >= 5:
            prev_x = []
