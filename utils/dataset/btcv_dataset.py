...
class BTCVDataset(NNUNetDataset):
    def __init__(self, base_dir: Path, dataset_type: Literal['train', 'valid', 'test'], transforms: transforms.Compose|None=transforms.Compose(transforms.Grayscale()), use_numpy=False, **kwargs):
        super(BTCVDataset, self).__init__(base_dir=base_dir, dataset_type=dataset_type, use_numpy=use_numpy, config_name='dataset_0.json')
    ...
    def __getitem__(self, index):
        ...
        # '.nii.gz'
        image = nib.load(image_file) # (C, H, W, D)
        mask = nib.load(label_file) # (C, H, W, D)
        ...
        if self.transforms:
            ...
            image_tensor = image_tensor.transpose(1, 2, 3, 0)
            mask_tensor = mask_tensor.transpose(1, 2, 3, 0)
        else:
